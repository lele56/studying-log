...
📌 测试环境：Python3.10 + LangChain1.0 + 通义千问qwen-plus + @tool装饰器（修复可调用问题）
{('='*90)}
【1. 携程机票预订结果】
{ctrip_result}
...
1. 调用【携程机票Agent】>>>
✅ 携程测试结果：
请问您需要预订从哪里出发，到哪里去的机票？另外，您的出行日期是哪一天？（日期格式如：2024-05-01）
--------------------------------------------------------------------------------
...
1. **城市**：您想住在哪个城市？
2. **附近地标**：您希望酒店靠近哪个地标或区域？
3. **入住日期**：您计划哪天入住？（格式：YYYY-MM-DD）
4. **离店日期**：您计划哪天离店？（格式：YYYY-MM-DD）
