...
import os # python 内置的「操作系统」模块，主要用来读取环境变量（如 API 密钥）

# load_dotenv: 从项目根目录的 .env 文件里，把变量加载到「环境」里，之后用 os.getenv("变量名") 进行读取
...
logging.basicConfig(
    ...
    format="%(asctime)s - %(levelname)s - %(message)s", # 日志格式：时间 - 日志级别 - 日志消息
)
logger = logging.getLogger(__name__) # 当前模块的 logger，方便在日志里看是哪个模块打印的
...
def init_llm_client():
    ...
    if not api_key or not model_name or not base_url:
        raise LangChainException("请确保已在 .env 文件中配置 OPENAI_API_KEY、OPENAI_MODEL_NAME 和 OPENAI_BASE_URL。")

    # 创建客户端：指定用哪个模型、密钥、接口地址（已经写到 .env 文件里面），以及「回复风格」相关参数。
    llm = ChatOpenAI(
        model=model_name,
        api_key=api_key,
        base_url=base_url,
...
def main():
    ...
    try:
        ...
        logger.info(f"回答: {response.content}")  # content 属性是纯文本格式
...
#   - 直接运行本文件时（如 python code.py），Python 会把 __name__ 设为字符串 "__main__"（前后各两个下划线），
#     于是下面的条件为真，会执行 main()；
#   - 被别的文件 import 时，__name__ 是模块名（如 "01_helloworld.code"），不等于 "__main__"，不会执行 main()，
#     避免一导入就自动跑一遍问问题。
#
...
"""
【输出示例】
2026-08-20 17:22:44,872 - INFO - LLM 客户端初始化成功！
2026-08-20 17:22:48,928 - INFO - HTTP Request: POST https://llm-3vs11lhgb27inah0.cn-beijing.maas.aliyuncs.com/compatible-mode/v1/chat/completions "HTTP/1.1 200 OK"
2026-08-20 17:22:48,941 - INFO - 问题: 你是谁
2026-08-20 17:22:48,941 - INFO - 回答: 我是通义千问，由阿里巴巴集团通义实验室自主研发的大语言模型。作为你的 AI 思考伙伴，我希望能为你提供清晰、准确且有帮助的解答，陪你一起探索问题、激发灵感。有什么我可以帮你的吗？
==================== 以下是流式输出（另一种调用方式）
**************************************************
2026-08-20 17:22:49,379 - INFO - HTTP Request: POST https://llm-3vs11lhgb27inah0.cn-beijing.maas.aliyuncs.com/compatible-mode/v1/chat/completions "HTTP/1.1 200 OK"
我是通义千问，由阿里巴巴集团通义实验室自主研发的大语言模型。很高兴能作为你的AI思考伙伴，有什么我可以帮你的吗？"""